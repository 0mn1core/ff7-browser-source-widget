 ...
 * Takes care of attaching the key from this function call to the component while passing
  ...
  const digitGroups = useMemo(() => {
    const orderedDigitGroups = digitRenderFunctions.map(
      (renderFunction, index) => {
        // Skip the active digit
        if (index === digit) {
          return null;
        }
        // Return the digit styled as inactive
        return renderFunction({ status: "inactive" });
      }
    );
    // Add the active digit last so it's rendered on top.
